...
import { Loader2, Package, Calendar, MapPin, TrendingUp, Leaf } from "lucide-react"
...
export function CollectionHistory({ collections, isLoading }: CollectionHistoryProps) {
  ...
  const totalCO2 = collections.reduce((sum, c) => sum + (Number(c.co2Saved) || (Number(c.weight) || 0) * 2.5), 0)
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="p-3 rounded-xl bg-primary/5 border border-primary/10">
                ...
                <p className="text-xl font-bold text-foreground">{totalWeight.toLocaleString()} kg</p>
              </div>
              <div className="p-3 rounded-xl bg-secondary/5 border border-secondary/10">
                <div className="flex items-center gap-2 mb-1">
                  <Leaf className="w-4 h-4 text-secondary" />
                  <span className="text-xs text-muted-foreground">CO2 Saved</span>
                </div>
                <p className="text-xl font-bold text-foreground">{Math.round(totalCO2).toLocaleString()} kg</p>
              </div>
            </div>
            ...
            <ScrollArea className="h-[280px] pr-4">
              <div className="space-y-3">
                {collections.map((collection, index) => {
                  ...
                  const co2 = Number(collection.co2Saved) || (Number(collection.weight) || 0) * 2.5
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <span className="text-lg font-bold text-foreground">{collection.weight || 0} kg</span>
                          <p className="text-xs text-secondary">{Math.round(co2)} kg CO2 saved</p>
                        </div>
                      </div>
                    </div>
                  )
