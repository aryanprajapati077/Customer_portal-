...
export function ImpactDashboardClient({ impact }: ImpactDashboardClientProps) {
  ...
  const impactMetrics = [
    ...
    {
      icon: TreePine,
      label: "Microplastic in tons",
      value: impact?.treesEquivalent || 225,
      suffix: " tons",
      change: "+15.2%",
      color: "text-accent",
    ...
    {
      icon: Wind,
      label: "Water Resources Protected in Litres",
      value: (impact?.co2Prevented || 5000) / 1000,
      suffix: "K L",
      change: "+9.7%",
      color: "text-primary",
      bgColor: "bg-primary/10",
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          {impactMetrics.map((metric, index) => (
            <Card
              key={metric.label}
              className={`p-5 sm:p-6 bg-card border-border/50 hover-lift group cursor-default transition-all duration-500 ease-out ${
